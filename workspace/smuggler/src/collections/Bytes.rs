...
pub(crate) trait Bytes
{
	...
	#[inline(always)]
	fn u8_slice(&self, index: u64, length: u64) -> Result<&[u8], OverflowError>
	{
		let pointer_to_index = self.pointer_to_index_length::<u8>(index, length)?;
		Ok(unsafe { from_raw_parts(pointer_to_index, u64_index_to_usize_index(length)) })
	}
	...
	#[inline(always)]
	fn unaligned_u16_native_endian_byte_order_value(&self, index: u64) -> Result<u16, OverflowError>
	{
	...
	#[inline(always)]
	fn unaligned_u16_value(&self, index: u64, byte_order: ByteOrder) -> Result<u16, OverflowError>
	{
		let pointer_to_index = self.pointer_to_index_checked::<u16>(index)?;
	...
	#[inline(always)]
	fn unaligned_u16_value_unchecked(&self, index: u64, byte_order: ByteOrder) -> u16
	{
	...
	#[inline(always)]
	fn unaligned_u32_value(&self, index: u64, byte_order: ByteOrder) -> Result<u32, OverflowError>
	{
	...
	#[inline(always)]
	fn unaligned_u32_value_unchecked(&self, index: u64, byte_order: ByteOrder) -> u32
	{
	...
	#[inline(always)]
	fn unaligned_u64_value(&self, index: u64, byte_order: ByteOrder) -> Result<u64, OverflowError>
	{
	...
	#[inline(always)]
	fn unaligned_u64_value_unchecked(&self, index: u64, byte_order: ByteOrder) -> u64
	{
