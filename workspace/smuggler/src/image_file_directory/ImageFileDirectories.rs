...
impl<A: Allocator + Clone, T: Tag> ImageFileDirectories<A, T>
{
	...
	pub(in crate::image_file_directory) fn parse<'tiff_bytes, 'allocator, TP: TagParser<'tiff_bytes, 'allocator, A, Tags<A, T>, T>, TB: TiffBytes, Unit: 'tiff_bytes + Version6OrBigTiffUnit>(common: &mut TagParserCommon<'tiff_bytes, 'allocator, TB, A, Unit>, zeroth_image_file_directory_pointer: ImageFileDirectoryPointer) -> Result<Self, ImageFileDirectoriesParseError>
	{
		...
		loop
		{
			...
			let (image_file_directory, next_image_file_directory_pointer) = ImageFileDirectory::parse::<TP, _, Unit>(common, image_file_directory_pointer).map_err(|cause| ImageFileDirectoryParse { cause, image_file_directory_pointer, index })?;
	...
	fn parse_child_image_file_directory<'tiff_bytes, 'allocator, TP: TagParser<'tiff_bytes, 'allocator, A, Tags<A, T>, T>, TB: TiffBytes, Unit: 'tiff_bytes + Version6OrBigTiffUnit>(common: &mut TagParserCommon<'tiff_bytes, 'allocator, TB, A, Unit>, vec_image_file_directories: &mut Vec<ImageFileDirectories<A, T>, A>, raw_offset: u64) -> Result<(), ChildImageFileDirectoriesParseError>
	{
		...
		let result = common.recurse(|common| Self::parse::<TP, TB, Unit>(common, zeroth_image_file_directory_pointer))?;
...
{
	#[inline(always)]
	pub(crate) fn parse_public_top_level<'allocator, TB: TiffBytes, Unit: 'tiff_bytes + Version6OrBigTiffUnit>(zeroth_image_file_directory_pointer: ImageFileDirectoryPointer, common: &mut TagParserCommon<'tiff_bytes, 'allocator, TB, A, Unit>) -> Result<Self, ImageFileDirectoriesParseError>
	{
		Self::parse::<PublicTagParser<'tiff_bytes, A>, _, Unit>(common, zeroth_image_file_directory_pointer)
	}
}
